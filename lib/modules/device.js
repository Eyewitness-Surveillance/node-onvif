...
function OnvifDevice(params) {
	...
	// allow non-local addressing
	// default to true
	this.isLocal = ('isLocal' in params) ? params.isLocal : true;
...
OnvifDevice.prototype._getXaddr = function (directXaddr) {
	// Adding support for non-local IPs, supplied in the xaddr
	if (!this.keepAddr) {
		if (!this.isLocal) {
			// We need to replace the IP and port in the directXaddr
			// with the original values from object creation
			let ourl = mUrl.parse(directXaddr);
			return 'http://' + this.oxaddr.host + ourl.path;
		}
		return directXaddr;
	}
